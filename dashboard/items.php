...
<div class="space-y-6">
    ...
    <div class="bg-gray-800 rounded-xl p-6 border border-gray-700">
        <h3 class="text-lg font-bold text-white mb-4"><i class="fas fa-plus-circle mr-2"></i>Add New Item</h3>
        <form method="POST" action="">
            <input type="hidden" name="action" value="add">
            <div class="grid grid-cols-1 md:grid-cols-2 gap-4">
                <div>
                    <label class="block text-gray-300 text-sm font-bold mb-2">Name</label>
                    <input type="text" name="name" required class="w-full bg-gray-700 text-white border border-gray-600 rounded-lg px-4 py-2 focus:outline-none focus:border-[#0038A8]" placeholder="Item name">
                </div>
                <div>
                    <label class="block text-gray-300 text-sm font-bold mb-2">Type</label>
                    <select name="type" required class="w-full bg-gray-700 text-white border border-gray-600 rounded-lg px-4 py-2 focus:outline-none focus:border-[#0038A8]">
                ...
                <div class="md:col-span-2">
                    ...
                    <textarea name="description" required rows="2" class="w-full bg-gray-700 text-white border border-gray-600 rounded-lg px-4 py-2 focus:outline-none focus:border-[#0038A8]" placeholder="Item description..."></textarea>
                </div>
                <div>
                    <label class="block text-gray-300 text-sm font-bold mb-2">Power</label>
                    <input type="number" name="power" value="0" min="0" class="w-full bg-gray-700 text-white border border-gray-600 rounded-lg px-4 py-2 focus:outline-none focus:border-[#0038A8]">
                </div>
                <div>
                    <label class="block text-gray-300 text-sm font-bold mb-2">Rarity</label>
                    <select name="rarity" required class="w-full bg-gray-700 text-white border border-gray-600 rounded-lg px-4 py-2 focus:outline-none focus:border-[#0038A8]">
            ...
            <table class="w-full">
                <thead class="bg-gray-700">
                    <tr>
                        ...
                        <th class="px-6 py-3 text-left text-xs font-medium text-gray-300 uppercase tracking-wider">Power</th>
                ...
                <tbody class="divide-y divide-gray-700">
                    <?php foreach ($items as $item): ?>
                        <tr class="hover:bg-gray-700/50">
                            <td class="px-6 py-4 whitespace-nowrap text-sm text-gray-300"><?php echo $item['id']; ?></td>
                            <td class="px-6 py-4 whitespace-nowrap text-sm font-medium text-white"><?php echo htmlspecialchars($item['name']); ?></td>
                            ...
                            <td class="px-6 py-4 text-sm text-gray-300"><?php echo htmlspecialchars(substr($item['description'], 0, 50)) . (strlen($item['description']) > 50 ? '...' : ''); ?></td>
                            <td class="px-6 py-4 whitespace-nowrap text-sm text-gray-300"><?php echo $item['power']; ?></td>
                            <td class="px-6 py-4 whitespace-nowrap">
                                <?php
                                $rarity_colors = [
                                    'common' => 'bg-gray-500',
